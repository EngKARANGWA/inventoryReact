...
const LatestPricesTable: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPrices = async () => {
      try {
        setIsLoading(true);
        const response = await fetch('https://test.gvibyequ.a2hosted.com/api/daily-price/latest-prices');
            ...
            <tr key={price.priceId}>
              <td className="px-6 py-4 whitespace-nowrap">
                ...
                {price.productDescription && (
                  <div className="text-sm text-gray-500">{price.productDescription}</div>
                )}
              </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {parseFloat(price.latestPrice).toLocaleString()}
              </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
